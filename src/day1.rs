pub fn solve_day1_part1() {
    let input_data: String = String::from(
        "29lzrxseven
9nnqljsixkzphvtmtr
...
3eighteightllkbxkbs9zgznxtj8lfflcst",
    ...
    let lines: Vec<String> = input_data.lines().map(|l| l.to_string()).collect();
    let mut calibration_codes: Vec<u32> = Vec::new();

    for l in lines {
        let mut code_from_line: String = String::new();
        for c in l.chars() {
            if c.is_numeric() {
                code_from_line.push(c);
                break;
            }
        }
        ...
        calibration_codes.push(code_from_line.parse::<u32>().unwrap());
    }
